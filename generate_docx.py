...
try:
    ...
    print("[-] Error: 'python-docx' library not found. Install it via: pip install python-docx")
    sys.exit(1)
...
def generate_docx(json_path: str, output_path: str):
    ...
    title_text = data.get("title", "Meeting Report")
    meeting_date = data.get("date", "June 09, 2026")
    exec_text = data.get("executiveSummary", "")
    ...
    meta_items = [
        ("Session Date", meeting_date),
        ("Primary Language", data.get("language", "en").upper()),
        ("Meeting Duration", duration_val),
        ("Total Participants", str(len(speakers))),
        ("Visual Slides Captured", str(len(slides))),
    ]
    ...
    if decisions:
        ...
        for dec in decisions:
            ...
            r = p.add_run(f"{dec.get('decision', '')} ({dec.get('owner', 'Everyone')})")
            r.font.size = Pt(10)
    ...
    if action_items:
        ...
        # Create Table (Task, Assignee, Priority, Deadline)
        ...
        headers = ["Action Task Description", "Assignee", "Priority", "Deadline"]
        ...
        for row_idx, item in enumerate(action_items):
            ...
            cells[1].text = item.get("owner", "Unassigned")
            
            prio = item.get("priority", "Medium").capitalize()
            cells[2].text = prio
            cells[3].text = item.get("deadline", "N/A")
        ...
        set_col_widths(action_table, [Inches(3.2), Inches(1.2), Inches(0.9), Inches(0.9)])
        doc.add_paragraph("")

    # ================================================================
    # KEY DISCUSSION TOPICS / KEY POINTS
    # ================================================================
    key_points = data.get("keyPoints", [])
    if key_points:
        add_heading_styled(doc, "Key Discussion Topics", level=1)
        for kp in key_points:
            add_heading_styled(doc, kp.get("category", "Topic"), level=2)
            p = doc.add_paragraph()
            p.paragraph_format.line_spacing = 1.15
            p.paragraph_format.space_after = Pt(8)
            r = p.add_run(kp.get("point", ""))
            r.font.size = Pt(10)
    ...
    # Get JSON folder parent to resolve slide image paths if relative
    ...
    for seg in segments:
        ...
        # Inline slide rendering if slide is linked to this segment
        if slide_id:
            # Find the slide details in the slides array
            slide_details = next((s for s in slides if s["id"] == slide_id), None)
            if slide_details:
                # Resolve relative URL dataUrl e.g. "/output/task_xxx/slides/slide_xxx.png"
                img_path = slide_details["dataUrl"]
                if img_path.startswith("/output/"):
                    # Translate to absolute path on disk: workspace_root/output/task_xxx/slides/slide_xxx.png
                    img_path = img_path.lstrip("/")
                    absolute_slide_path = os.path.join(os.getcwd(), img_path)
                else:
                    absolute_slide_path = img_path
                    
                if absolute_slide_path != last_slide_embedded and os.path.isfile(absolute_slide_path):
                    # Add caption
                    ...
                    cap_run = caption_p.add_run(f"🖼️ [{slide_details.get('caption', 'Active Slide')}]")
                    ...
                    # Embed Image
                    try:
                        doc.add_picture(absolute_slide_path, width=Inches(4.8))
                        img_p = doc.paragraphs[-1]
                        img_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        img_p.paragraph_format.space_after = Pt(12)
                        last_slide_embedded = absolute_slide_path
                    except Exception as e:
                        print(f"⚠️ Failed to insert image {absolute_slide_path}: {e}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python generate_docx.py <report.json> [output.docx]")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        out_path = json_path.replace("report.json", "report.docx").replace(".json", ".docx")
